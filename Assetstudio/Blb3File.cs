...
namespace AssetStudio
{
    public class Blb3File
    {
        ...
        private void ReadBlocksInfoAndDirectory(byte[] header)
        {
            ...
            Logger.Verbose($"目录数量:{nodesCount}");
        ...
        private Stream CreateBlocksStream(string path)
        {
            ...
            Logger.Verbose($"解压后数据块的总大小: 0x{uncompressedSizeSum:X8}");
                    ...
                    case CompressionType.Oodle:
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    Logger.Warning($"Oodle解压错误,写入{numWrite}字节,但预期写入{uncompressedSize}字节");
                                }
                            }
                            finally
                            {
                    ...
                    case CompressionType.Lz4HC: //LZ4HC
                        {
                            ...
                            try
                            {
                                ...
                                if (numWrite != uncompressedSize)
                                {
                                    Logger.Warning($"Lz4解压错误,写入{numWrite}字节,但预期写入{uncompressedSize}字节");
                                }
                            }
                            catch (Exception e)
                            {
                                Logger.Error($"Lz4解压错误{e.Message}");
                            }
                            finally
                            {
                                blocksStream.Write(uncompressedBytesSpan);
